"""Allow two active committee accounts to share one officer roster identity."""

from collections.abc import Sequence

from alembic import op

revision: str = "948e359c2344"
down_revision: str | None = "972c30a34f67"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None


CAP_SQL = """
DROP INDEX public.profiles_active_officer_identity;

CREATE FUNCTION public.limit_officer_accounts_to_two()
RETURNS trigger
LANGUAGE plpgsql SET search_path = public, pg_temp AS $$
BEGIN
  IF NEW.officer_id IS NULL OR NOT NEW.active THEN RETURN NEW; END IF;
  IF TG_OP = 'UPDATE' THEN
    IF NEW.officer_id IS NOT DISTINCT FROM OLD.officer_id
       AND NEW.active IS NOT DISTINCT FROM OLD.active THEN
      RETURN NEW;
    END IF;
  END IF;

  -- Serialize links to the same role before counting; two simultaneous
  -- requests cannot both claim its second available slot.
  PERFORM 1 FROM public.officers WHERE id = NEW.officer_id AND active FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'active officer not found'; END IF;
  IF (SELECT count(*) FROM public.profiles
      WHERE officer_id = NEW.officer_id AND active AND id <> NEW.id) >= 2 THEN
    RAISE EXCEPTION 'officer account limit reached';
  END IF;
  RETURN NEW;
END;
$$;

CREATE TRIGGER limit_officer_accounts_to_two
BEFORE INSERT OR UPDATE OF officer_id, active ON public.profiles
FOR EACH ROW EXECUTE FUNCTION public.limit_officer_accounts_to_two();
"""
  ...
  CASE p_action
    ...
    WHEN 'link_officer' THEN
      ...
      IF p_officer_id IS NOT NULL THEN
        ...
        -- The profile trigger serializes and enforces the two-account cap.
      END IF;
...
def upgrade() -> None:
    op.execute(CAP_SQL)
    op.execute(REVIEW_SQL)


def downgrade() -> None:
    raise RuntimeError("Officer account limit changes require a reviewed forward migration")
