...
def fixture_session():
    ...
    session.get.side_effect = [
        ...
        SlackChannelSyncSetting(channel_id="C1", enabled=True, last_successful_sync_at="2026-09-23"),
    ]
    source = [
    ...
    session.scalars.side_effect = [Mock(all=Mock(return_value=source)), Mock(all=Mock(return_value=reactions))]
    return session
...
def test_check_on_repost_overrides_archived_x():
    ...
    assert [ticket.ticket_id for ticket in plan_reposts(fixture_session(), client, "C1", now=2_000_000)] == ["ACTION-003"]
