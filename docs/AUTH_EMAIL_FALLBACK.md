...
At 15:06 UTC on 24 September, a controlled owner setup request was accepted by Resend; the owner confirmed the received email contains both the code and code-page link, without sharing either credential. A Shashwat request at 15:07 UTC returned 503 before Supabase updated `confirmation_sent_at`; its ledger row remained `sending` with no provider ID, so no delivery conclusion can be drawn from it. One retry at 15:09 UTC was accepted by Resend and advanced Shashwat's confirmation time. Tanuj's request at 15:08 UTC was also accepted and advanced his confirmation time. Both accounts remained unconfirmed immediately after these sends. Resend acceptance does not establish inbox placement or successful confirmation; their current result is pending.
