...
Later on 24 September, a new setup email for `shashwat.sarawagi25@imperial.ac.uk` was accepted at 10:08 UTC and another for `srs25@ic.ac.uk` at 10:10 UTC. Screenshots showed the setup confirmation page and an expired/used-link message after Continue. The user reports Shashwat opened the newest email and requested further links around 11:08 and 11:10 BST. The database confirms one successful Shashwat send at 11:08 BST; the 11:10 BST confirmation timestamp belongs to `srs25@ic.ac.uk`, so a second Shashwat send is not established. Read-only Auth checks still found both accounts unconfirmed, without sessions or site profiles. The site now preserves the requested setup/reset/link mode after a failed confirmation and records only the safe Supabase error code, flow and status in server logs; it never logs links, tokens or addresses. After this deployment, make one fresh setup request for Shashwat, open that specific email promptly, and inspect the safe error code if Continue still fails. Do not keep sending links without checking the newest one; a new request may invalidate an older link.
