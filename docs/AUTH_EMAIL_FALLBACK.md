...
All normal action emails (signup, invitation, magic link, recovery, email change, reauthentication) are handled, together with standard security-change notifications. Signup, invitation, magic-link and recovery emails now open the site's scanner-resistant `/auth/confirm` page before a human POST verifies the token. Secure email change currently uses Supabase's direct verification route with the documented old/new hash mapping; scanner-resistant confirmation for that flow remains to be verified and implemented.
