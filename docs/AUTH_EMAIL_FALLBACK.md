...
`send-auth-email` version 6 is deployed in project `immldithmugfrpojetmm`, and its Auth Hook is active. Fresh signup, setup, magic-link and recovery emails now include both a link and a numeric one-time code. The code can be entered at `/auth/verify-code` if the link fails. Existing emails sent before version 6 contain no code; a new request replaces the previous link and code. The code page and hook passed local tests, and the deployed page and hook health endpoint were checked on 24 September 2026. Completion with a real new code remains unverified.

At 15:06 UTC on 24 September, a controlled owner setup request was accepted by Resend; the owner confirmed the received email contains both the code and code-page link, without sharing either credential. A Shashwat request at 15:07 UTC returned 503 before Supabase updated `confirmation_sent_at`; its ledger row remained `sending` with no provider ID, so no delivery conclusion can be drawn from it. One retry at 15:09 UTC was accepted by Resend and advanced Shashwat's confirmation time. Supabase then recorded Shashwat's email confirmation and sign-in at 15:11 UTC and created his active `member` profile. A normal subsequent password sign-in has not yet been reported. Tanuj's request at 15:08 UTC was accepted and advanced his confirmation time, but his account remained unconfirmed at the latest check. Resend acceptance does not establish inbox placement; Tanuj's result is pending.

A controlled password-reset email reached the account owner on 24 September 2026; its fresh link opened the correct **Set your password** page. Resend showed **Delivered** for two other signup recipients who reported no inbox arrival. One fresh setup email was requested for each on 24 September at 06:46 UTC; Supabase recorded new confirmation send times and the hook recorded two Resend acceptances. Their inbox outcomes and completed confirmations remain unverified. Brevo fallback and provider delivery callbacks still need live verification. The preserved Resend SMTP configuration is the rollback path if the hook fails.

Later on 24 September, a new setup email for `shashwat.sarawagi25@imperial.ac.uk` was accepted at 10:08 UTC and another for `srs25@ic.ac.uk` at 10:10 UTC. Screenshots showed the setup confirmation page and an expired/used-link message after Continue. The user reports Shashwat opened the newest email and requested further links around 11:08 and 11:10 BST. The database confirms one successful Shashwat send at 11:08 BST; the 11:10 BST confirmation timestamp belongs to `srs25@ic.ac.uk`, so a second Shashwat send is not established. Read-only Auth checks at 14:48 UTC still found Shashwat and Tanuj unconfirmed, without sessions or site profiles; their latest recorded emails were at 11:37 and 14:26 UTC respectively. The site preserves the requested setup/reset/link mode after a failed confirmation and records only the safe Supabase error code, flow and status in server logs; it never logs links, tokens or addresses. The new code option gives them a second method after one fresh email. Do not keep sending links without checking the newest one; a new request invalidates an older link and code.
...
Supabase Auth also applies limits **before** the Send Email hook. Its [current rate-limit reference](https://supabase.com/docs/guides/auth/rate-limits) distinguishes a project-wide auth-email quota from per-user request windows and IP-based signup/sign-in limits. With a custom SMTP provider or Send Email hook, the project-wide email quota is configurable; the built-in provider's published default is two emails per hour. The user-level signup, reset and magic-link windows default to 60 seconds, and signup/sign-in IP traffic has a separate default of 30 requests per five minutes with a burst allowance. A 429 at this layer can prevent the hook from running at all, so an empty EFDS delivery ledger does **not** establish that a request reached Resend or Brevo. The site gives a safe rate-limit message and has a 60-second per-email resend cooldown, but those controls do not replace Auth's server-side limits. Check this project's actual values at **Supabase → Authentication → Rate Limits**; they were not readable in this session. Keep the configured project-wide hourly limit consistent with both providers' capacity and do not raise it solely to work around the two missing Imperial inbox messages.
