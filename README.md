...
If a fresh crawler run reports failed pages, do not run an unrestricted sync from that incomplete archive: it could mark older articles missing even when the crawler merely failed to fetch them. First count the current `data/articles/**/metadata.json` files, run a dry comparison, and pass that count as `--max-articles N` for the import. The limit processes the captured articles but disables missing-source reconciliation. Resolve the crawl failures and perform an unrestricted sync only after a complete crawl. On 24 September 2026, four publicly linked ICU pages returned HTTP 404; the 90 successfully crawled articles were imported with `--max-articles 90`, leaving older records and their review state intact.
